...
Optional: `northstar practice walk --save` writes a local lesson notebook (`~/.poly/walk_journal.json`). That is not a trade. Read it with `northstar practice journal`. Replay the newest lesson with `northstar practice last`. Machine-readable dump: `northstar practice last --json` or `northstar practice journal --json`. Read paper fills (not the walk journal) with `northstar practice paper list`. Paper hedge: `northstar practice paper book --both` only when the pair is under $1; settle YES with the real outcome and NO with the opposite. Orientation: `northstar status` (or `--json`).
