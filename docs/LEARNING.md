...
Optional: `northstar practice walk --save` writes a local lesson notebook (`~/.poly/walk_journal.json`). That is not a trade. Read it with `northstar practice journal`. Replay the newest lesson with `northstar practice last`. Machine-readable dump: `northstar practice last --json` or `northstar practice journal --json`. Read paper fills (not the walk journal) with `northstar practice paper list`. Orientation: `northstar status` (or `--json`).
